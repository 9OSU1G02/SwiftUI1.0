...
//  SwiftUI1.0
//
//  Created by Quốc Huy Nguyễn on 7/2/23.
//
                        ...
                        ForEach(courses.indices, id: \.self) { index in
                            GeometryReader { geometry in
                                CourseView(
                                    ...
                                    activeView: self.$activeView, bounds: proxy
                                )
                                    .offset(y: self.courses[index].show ? -geometry.frame(in: .global).minY : 0)
                                    .opacity(self.activeIndex != index && self.active ? 0 : 1)
                                    .scaleEffect(self.activeIndex != index && self.active ? 0.5 : 1)
                                    .offset(x: self.activeIndex != index && self.active ? proxy.size.width : 0)
                            }
                            .frame(height: 280)
                            .frame(maxWidth: self.courses[index].show ? 712 : getCardWidth(bounds: proxy))
